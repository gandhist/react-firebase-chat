    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            firebase.database().ref('chats/').orderByChild('roomname').equalTo(roomname).on('value', resp => {
                setChats([])
                setChats(snapShootToArray(resp))
            })
        }
    ...
    const exitChat = (e) => {
        ...
        firebase.database().ref('roomusers/').orderByChild('roomname').equalTo(roomname).once('value').then(
            (resp) => {
                ...
                if (user !== undefined) {
                    const userRef = firebase.database().ref('roomusers/' + user.key).update({ status: 'offline', src: 'exit', exitAt: Moment(new Date()).format('DD/MM/YYYY HH:mm:ss') });
